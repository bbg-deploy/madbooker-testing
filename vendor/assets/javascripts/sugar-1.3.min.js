...
var Qb=[],Rb=[],Y=[],Sb=[],Tb={},Ub,Z;function Vb(a,b){var c=a.indexOf(b);c>-1&&a.splice(c,1)}
function Wb(a,b,c){C(b)&&Vb(Y,b);Vb(Y,c);a.unshift({ka:b,pa:c})}function Xb(a,b){return a==b||a=="all"||!a}function Yb(a){return Y.some(function(b){return(new r("\\b"+b+"$","i")).test(a)})}function Zb(a,b){a=C(a)?a.toString():"";return a.isBlank()||Yb(a)?a:$b(a,b?Qb:Rb)}function $b(a,b){G(b,function(c,d){if(a.match(d.ka)){a=a.replace(d.ka,d.pa);return n}});return a}function ac(a){return a.replace(/^\W*[a-z]/,function(b){return b.toUpperCase()})}
Z={acronym:function(a){Tb[a.toLowerCase()]=a;a=p.keys(Tb).map(function(b){return Tb[b]});Z.acronymRegExp=r(a.join("|"),"g")},plural:function(a,b){Wb(Qb,a,b)},singular:function(a,b){Wb(Rb,a,b)},irregular:function(a,b){var c=a.first(),d=a.from(1),e=b.first(),f=b.from(1),h=e.toUpperCase(),i=e.toLowerCase(),j=c.toUpperCase(),g=c.toLowerCase();Vb(Y,a);Vb(Y,b);if(j==h){Z.plural(new r("({1}){2}$".assign(c,d),"i"),"$1"+f);Z.plural(new r("({1}){2}$".assign(e,f),"i"),"$1"+f);Z.singular(new r("({1}){2}$".assign(e,
f),"i"),"$1"+d)}else{Z.plural(new r("{1}{2}$".assign(j,d)),h+f);Z.plural(new r("{1}{2}$".assign(g,d)),i+f);Z.plural(new r("{1}{2}$".assign(h,f)),h+f);Z.plural(new r("{1}{2}$".assign(i,f)),i+f);Z.singular(new r("{1}{2}$".assign(h,f)),j+d);Z.singular(new r("{1}{2}$".assign(i,f)),g+d)}},uncountable:function(a){var b=q.isArray(a)?a:K(arguments);Y=Y.concat(b)},human:function(a,b){Sb.unshift({ka:a,pa:b})},clear:function(a){if(Xb(a,"singulars"))Rb=[];if(Xb(a,"plurals"))Qb=[];if(Xb(a,"uncountables"))Y=[];
if(Xb(a,"humans"))Sb=[];if(Xb(a,"acronyms"))Tb={}}};Ub=["and","or","nor","a","an","the","so","but","to","of","at","by","from","into","on","onto","off","out","in","over","with","for"];Z.plural(/$/,"s");Z.plural(/s$/gi,"s");Z.plural(/(ax|test)is$/gi,"$1es");Z.plural(/(octop|vir|fung|foc|radi|alumn)(i|us)$/gi,"$1i");Z.plural(/(census|alias|status)$/gi,"$1es");Z.plural(/(bu)s$/gi,"$1ses");Z.plural(/(buffal|tomat)o$/gi,"$1oes");Z.plural(/([ti])um$/gi,"$1a");Z.plural(/([ti])a$/gi,"$1a");
...
F(t,k,n,{pluralize:function(){return Zb(this,k)},singularize:function(){return Zb(this,n)},humanize:function(){var a=$b(this,Sb),b;a=a.replace(/_id$/g,"");a=a.replace(/(_)?([a-z\d]*)/gi,function(c,d,e){b=H(Tb,e)?Tb[e]:l;return(d?" ":"")+(b||e.toLowerCase())});return ac(a)},titleize:function(){var a=/[.:;!]$/,b,c,d;return this.spacify().humanize().words(function(e,f,h){b=a.test(e);d=f==0||f==h.length-1||b||c;c=b;return d||Ub.indexOf(e)===-1?ac(e):e}).join(" ")},parameterize:function(a){var b=this;
if(a===undefined)a="-";if(b.normalize)b=b.normalize();b=b.replace(/[^a-z0-9\-_]+/gi,a);if(a)b=b.replace(new r("^{sep}+|{sep}+$|({sep}){sep}+".assign({sep:R(a)}),"g"),"$1");return encodeURI(b.toLowerCase())}});t.Inflector=Z;t.Inflector.acronyms=Tb;})();